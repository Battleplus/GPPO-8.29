...
| 四学习方法 training runner smoke | 8 个同 seed 微型 run；真实 optimizer updates、checkpoint/RNG/SHA seal PASS；正式训练未启动 |
| 正式训练 | 未启动 |
...
- 三种正式非学习基线已通过 600 次安全回放；该开发带主要覆盖机制边界，算法效果差异仍需在正式训练封存后的独立 Hidden-V1 上判断；
- 图 schema、reward、指标、训练合同、统一 PPO/GPPO adapter 和 Gym/PyTorch 确定性短回放已经完成；本机 PyG 为非必需可选依赖，因 NumPy/Pandas 二进制不兼容无法原生导入；
...
> 已经证明执行中抢占与动态重分配机制在十类极端事件下能够保持安全、一致和可追溯；已经完成 PPO/GPPO 共享接口、训练器、checkpoint/RNG evidence seal、三个非学习基线和独立 Gate 机制。尚未证明任何学习算法在新合同下优于 PPO；下一步从最终远端 source SHA 生成 evidence-only Gate，之后才能重新训练并进入独立 hidden 验证。
...
2. **证据进展：** 十类场景、200 条开发事件带全部通过规则回放；学姐旧方法适配、Greedy Priority 与 Beam-MPC 共 600 次同带安全回放，没有资源冲突或事务不变量失败；4/8/16/32 UAV 图构建 smoke 已跑通。
3. **结论边界：** 目前完成的是机制、接口、训练合同、训练器短回放、三个非学习基线和 Gate 实现，不是模型胜负。下一阶段只生成 source-bound evidence；只有重新训练和独立 hidden 结果出来后才讨论谁超过 PPO。
